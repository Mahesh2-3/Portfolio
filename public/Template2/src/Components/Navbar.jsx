...
const Navbar = () => {
  ...
  return (
    <nav className="w-full fixed top-0 z-20 h-[70px] flex items-center bg-black/10 backdrop-blur-md justify-between px-8 font-primary">
      ...
      {/* Mobile Menu */}
      {menuOpen && (
        <div className="absolute top-[70px] right-0 bg-gradient-to-br from-light/20 via-light/40 to-light/80 backdrop-blur-sm rounded-2xl mr-5 w-fit h-fit p-8 flex flex-col items-center justify-start pt-10 gap-6 text-white">
          {Navlinks.map((link, index) => (
            <a
              key={index}
              href={link.src}
